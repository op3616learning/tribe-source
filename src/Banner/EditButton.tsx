...
export const EditButton = ({ onRemove }) => {
    const { toggleEditing, isEditing, onImageSelect } = useContext(bannerContext);
    const { isMobile } = useResponsive();
    const [buttonBorderRadius] = useToken('radiuses', ['base']);
    if (isEditing)
        return null;
    return (React.createElement(Dropdown, { placement: "bottom-end" }, ({ onClose }) => (React.createElement(React.Fragment, null,
        ...
        React.createElement(DropdownList, { zIndex: "dropdown" },
            React.createElement(DropdownItem, { "data-testid": "upload-image-button", as: FormLabel, icon: Image2LineIcon, mr: 0, mb: 0, cursor: "pointer", skipClickEvent: isMobile, htmlFor: "banner-image-upload-input" },
                React.createElement(Input, { type: "file", onChange: event => {
                        onImageSelect(event);
                        onClose();
                    }, accept: IMAGE_TYPES, display: "none", id: "banner-image-upload-input" }),
