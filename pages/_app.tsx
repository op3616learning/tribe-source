...
import { AppPageProps, PageWithLayout } from '@types'
      ...
      <TribeOptimizelyProvider
        ...
        user={{
          ...
          attributes: {
            ...
            clusterName: getRuntimeConfigVariable('SHARED_CLUSTER_NAME') || '',
            serviceName: getRuntimeConfigVariable('SHARED_SERVICE_NAME') || '',
          },
        }}
      >
...
NextApp.getInitialProps = async ({ Component, ctx, router }) => {
  try {
    ...
    let appProps
    const url = isServer ? ctx?.req?.url : window.location.pathname
    const isSpaceRoute = match('/:slug')(url)
    const isValidSpaceRoute = match('/:slug([A-Za-z0-9-]+)')(url)

    const isBot = isSpaceRoute && !isValidSpaceRoute

    // if the page is standAlone then it does not need appsProps
    if (!isBot && Component.standAlone !== true && !ctx?.res?.writableEnded) {
      appProps = await getAppInitialProps(ctx, Component.options, router)
    }
    if (!isBot && Component.getInitialProps && !ctx?.res?.writableEnded) {
