...
import { Post, Space } from 'tribe-api/interfaces'
    ...
    apolloClient
      ...
      .then(res => {
        try {
          ...
          const { space } = post || {}
          const idToRemove = post?.id

          apolloClient.cache.modify({
            id: apolloClient.cache.identify(space as Space),
            fields: {
              pinnedPosts: (cachedPinnedPostsRef, { readField }) => {
                return cachedPinnedPostsRef.filter(
