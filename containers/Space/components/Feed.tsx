...
const Feed = ({
  ...
  const postsIdMapper = useMemo(() => {
    return posts?.reduce((mapper, post: Post) => {
      return {
        ...mapper,
        [post.id]: true,
      }
  ...
  // Only shortlist pinned posts that are not present in posts.
  // Aids rendering of pin/unpin while changing cache.
  const shortlistedPinnedPosts: Array<Post> = useMemo(
    () => pinnedPosts.filter(post => !postsIdMapper[post.id]),
    [pinnedPosts, postsIdMapper],
  )

  const postFeed = useMemo(
    () => shortlistedPinnedPosts.concat(posts.sort(sortPosts)),
    [posts, shortlistedPinnedPosts],
  )
