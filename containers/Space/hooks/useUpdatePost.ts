...
const useUpdatePost = (
  ...
  const update = useCallback(
    (title: string, content: string, otherFields = { tagNames: [] }) => {
      ...
      updatePost({
        ...
        optimisticResponse: {
          __typename: 'Mutation',
          updatePost: merge({}, post, { mappingFields, ...otherFields }),
        },
        update: (cache, { data: fetchedData }) => {
          try {
            ...
            if (
              ...
              toast({
                title: t('post:feedback.blocked.title', 'Pending review'),
                description: t(
                  'post:feedback.blocked.description',
