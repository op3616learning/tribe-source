...
export const useRemoveSpaceMember: UseRemoveSpaceMemberProps = space => {
  ...
  const updateSpace = (
    ...
    const { space: spaceFromCache } =
      cache.readQuery<SpaceQuery>({
        query: SPACE,
        variables: { id: space.id, slug: space.slug },
      }) || {}
    if (spaceFromCache) {
      cache.writeFragment({
        ...
        `,
        data: {
          membersCount: Math.max(spaceFromCache?.membersCount - 1, 0),
        },
      })
    } else {
      logger.error('Space not found in cache for useRemoveSpaceMember')
    }
  }
