...
import { UpdatePostInput } from 'tribe-api'
import {
  AddPostMutation,
...
const useAddPost = (
  ...
  const updateFeed = (
    ...
    try {
      ...
      logger.warn('getFeed query cache update warn => ', error)
    }
  }
  ...
  const publish = useCallback(
    (
      ...
      title,
      content,
      otherFields: UpdatePostInput = { tagNames: [] },
    ) => {
      ...
      const optimisticPost: AddPostMutation['addPost'] = {
        ...
        isHidden: false,
        imageIds: [],
        attachmentIds: otherFields.attachmentIds || [],
        attachments: [],
        authMemberProps: {
      ...
      return addPost({
        ...
        update: (cache: ApolloCache<AddPostMutation>, { data }) => {
          ...
          try {
            ...
            updatePosts(cache, post as Post)
          } catch (e) {
            logger.error('error - updating GET_POSTS for useAddPost', e)
          }

          try {
            updateFeed(cache, post as Post, isGuest)
          } catch (e) {
