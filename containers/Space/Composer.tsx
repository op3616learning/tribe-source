...
import { Box, Container, HStack, useToken } from '@chakra-ui/react'
...
import { useReportAttachmentDownload } from 'containers/Post/hooks/useReportAttachmentDownload'
...
export const COMPOSER_SPACING = 6
...
const Composer = ({
  ...
  const onAttachmentDownload = useReportAttachmentDownload(post || null)
  ...
  } = useAttachments(getPostAttachments(post))
  const isUploadingAttachment = attachments.some(({ isLoading }) => isLoading)
  ...
  let postType = space?.authMemberProps?.availablePostTypes?.find(
    type => type.name === 'Discussion',
  ) as PostType
  if (!postType) {
    postType = space?.authMemberProps?.availablePostTypes?.[0] as PostType
  }
  ...
  }, [closeModal, clearAttachments, onCloseCallback])
  ...
  const close = useCallback(() => {
    // Prevent closing modal if there's a new attachment
    const haveNewAttachment = attachments.some(({ isNew }) => isNew)
  ...
  }, [attachments, completeClose, confirmClose, hasPostTitleChanged])
  ...
  const submit = useCallback(() => {
    ...
    const otherFields = {
      tagNames: updatedTopics || [],
      attachmentIds: attachments.map(({ id }) => id),
    }

    if (isNew && postType) {
      publish(postType, title, _content, otherFields)
      completeClose()
    } else {
      update(title, _content, otherFields)
    }
    resetTopicsState()
  }, [
    isNew,
