...
import { AdminSidebarBackButton } from './AdminSidebarBackButton'

const BrandingItem = memo<{ href: string; active: boolean }>(
  ({ href, active }) => {
    const { isNeo } = useNeo()

    if (isNeo !== false) return null
    ...
    return (
      <NextLink passHref href={href}>
        <SidebarItem
          ...
          icon={BrushFillIcon}
        >
          <Trans i18nKey="admin:sidebar.items.theme" defaults="Branding" />
        </SidebarItem>
      </NextLink>
    )
  },
...
const AdminSidebar = () => {
  ...
  const spacePostTypeAppSlug =
    space?.authMemberProps?.availablePostTypes?.[0]?.name === 'Discussion'
  ...
  const { authorized: hasUpdateNetworkPermission } = hasActionPermission(
  ...
  return (
    <>
      <Sidebar pt={{ base: '5rem', [SIDEBAR_VISIBLE]: 8 }} pb={6}>
        <AdminSidebarBackButton
          spaceSlug={spaceSlug}
          name={spaceSlug ? space?.name : network?.name}
        />
        ...
        {isNetworkAdmin && (
          <>
            ...
            {isNetworkSetting && (
              <BrandingItem
                href={getAs('branding', query['space-slug'])}
        ...
        <Spacer mt={5} />
        {hasUpdateNetworkPermission && <SidebarPlanBar />}
      </Sidebar>
