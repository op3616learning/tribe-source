...
import { Maybe, Media, Image } from 'tribe-api/interfaces'
import {
  Avatar,
  Button,
  ButtonProps,
...
export const SpacesDropdown = () => {
  ...
  const {
    // All spaces even those that auth user can't update
    spaces: allSpaces,
    query: spacesQuery,
      ...
      <Popover isOpen={isOpen} onClose={close} isLazy placement="bottom-end">
        <PopoverTrigger>
          <Button {...staticProps.dropdownButton} onClick={open}>
            <SettingsTypeButton
              icon={
                (space ? (space?.image as Media) : network?.favicon) ||
                undefined
              }
          ...
          <Box overflowY="auto" maxHeight="xl" p={2}>
            ...
            {spaces.length > 0 ? (
              spaces.map(({ slug, name, image }) => (
                ...
                >
                  <Avatar src={image as Image} name={name} size="sm" />
                  <Text
                    ml={2}
                    color="label.primary"
