...
import { SpacePermissionSettings } from './SpacePermissionsSettings'
...
const SpaceGeneralSettings: FC<{ slug: Space['slug'] }> = ({ slug }) => {
  ...
  const {
    isEnabled: hasSpacePostingPermissionsFeatureEnabled,
  } = useTribeFeature(Features.SpacePostingPermissions)
  ...
  return (
    <SkeletonProvider loading={loading}>
      <Box>
        ...
        <Access space={space} />
        {hasSpacePostingPermissionsFeatureEnabled &&
          Object.prototype.hasOwnProperty.call(space, 'whoCanPost') && (
            <SpacePermissionSettings space={space} key={space?.id} />
          )}
