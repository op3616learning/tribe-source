...
const SlashMenu = ({ quill }: SlashMenuProps) => {
  ...
  const removeTextAfterSlash = useCallback(() => {
    ...
    // Last line till cursor position
    const line = quill.getContents(0, cursorPosition).ops.slice(-1)[0].insert

    const slashTextSize = line.slice(line.lastIndexOf(SLASH_SYMBOL)).length

    // Delete slash search completely
    quill.deleteText(cursorPosition - slashTextSize, slashTextSize)
  }, [quill])

  const onMenuItemClick = useCallback(
    (tag: string) => {
      if (filteredItems) {
        removeTextAfterSlash()
      }
